...
import { useCoreContract, usePlanetName, useSelectedPlanet, capturePlanets } from "../lib/darkforest";
...
export function MoveView(): JSX.Element {
  ...
  const submitMoveBundle = async () => {
    ...
    for (let i = 0; i < state.planets.length; i++) {
      // @ts-expect-error
      const p = await df.getPlanetWithId(state.planets[i]) as Planet;
      const moveTxArray = await capturePlanets(p.locationId,1,3,75,0,10,contract)
      console.log(moveTxArray);
      // @ts-expect-error
      df.terminal.current.println(`bundlign ${moveTxArray.length} moves...`);
      for(let tx of moveTxArray) {
        transactions.push({
          to: contract.address,
          transaction: tx,
        });
  ...
  if (flashbotsError) {
    ...
    content = (
      <div>
        <div>
          <span style={descriptionStyle}> Make multiple moves in a single block.</span>
        </div>
        ...
        <div style={actionContainerStyle}>
          ...
          <button disabled={state.planets.length === 0} onClick={submitMoveBundle}>
            Flashbots move
          </button>
        </div>
      </div>
